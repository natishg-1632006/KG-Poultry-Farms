...
const PaginationControls = ({ currentPage, totalItems, itemsPerPage = ITEMS_PER_PAGE, onPageChange }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-slate-100 print:hidden text-xs">
      <div className="text-slate-500 font-semibold shrink-0">
        Showing <strong className="text-slate-900 font-extrabold">{startItem}</strong> to <strong className="text-slate-900 font-extrabold">{endItem}</strong> of <strong className="text-slate-900 font-extrabold">{totalItems}</strong> entries
      </div>
      <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none whitespace-nowrap max-w-full pb-0.5 shrink-0">
        <button
          type="button"
          onClick={() => handlePageClick(currentPage - 1)}
          disabled={currentPage === 1}
          className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-40 disabled:hover:bg-white transition-all active:scale-95 cursor-pointer shadow-2xs shrink-0 whitespace-nowrap"
        >
          Previous
        </button>
        ...
        {pages.map((pg, i) =>
          pg === '...' ? (
            <span key={`ellipsis-${i}`} className="px-1 text-slate-400 font-bold select-none shrink-0">
              ...
            </span>
          ) : (
            <button
              ...
              className={`h-8 min-w-[32px] px-2 rounded-xl text-xs font-black transition-all cursor-pointer shrink-0 whitespace-nowrap ${
                currentPage === pg
      ...
      <div className="space-y-6 pb-12 print:space-y-4 print:pb-0">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 print:hidden">
          <button
            ...
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-100 hover:bg-slate-200 px-4 py-2.5 text-xs font-bold text-slate-800 transition-all active:scale-95 cursor-pointer shadow-2xs w-fit"
          >
            <ArrowLeft className="h-4 w-4" />
            <span>Back to Batch History</span>
          </button>

          {/* Equal Width Side-by-Side Action Buttons */}
          <div className="grid grid-cols-2 gap-3 w-full sm:w-auto min-w-[280px] sm:min-w-[340px]">
            <button
              onClick={handlePrint}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-slate-800 transition-all active:scale-95 cursor-pointer w-full"
            >
        ...
        {/* Dispatches List */}
        <div id="trader-detail-section" className="scroll-mt-20 space-y-6">
          {traderDispatches.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center text-xs text-slate-400">
              No detailed dispatches found for this trader in this batch.
            </div>
          ) : (
              ...
              return (
                <div key={dIdx} className="space-y-4">
                  ...
                  <div className="px-1 text-xs font-semibold text-slate-500">
                    Vehicle #: <strong className="text-slate-900">{disp.vehicleNumber || disp.vehicleName || 'N/A'}</strong> • Driver: <strong className="text-slate-900">{disp.driverName || 'N/A'}</strong> {disp.driverMobileNumber ? `(${disp.driverMobileNumber})` : ''} • Date: <strong className="text-slate-900">{disp.dispatchDate}</strong>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">TOTAL NET WEIGHT</span>
                      <div className="mt-1 flex items-baseline gap-1">
                        <span className="text-2xl font-black text-slate-900">{totalNet.toFixed(1)}</span>
                    ...
                    {/* Birds Dispatched */}
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">BIRDS DISPATCHED</span>
                      <div className="mt-1 flex items-baseline gap-1">
                        <span className="text-2xl font-black text-emerald-800">{totalChicks}</span>
                        <span className="text-xs font-semibold text-slate-500">birds</span>
                      </div>
                    </div>

                    {/* Average Bird Weight */}
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">AVERAGE BIRD WEIGHT</span>
                      <div className="mt-1 flex items-baseline gap-1">
                        <span className="text-2xl font-black text-emerald-700">{avgWt}</span>
                        <span className="text-xs font-semibold text-slate-500">kg/bird</span>
                      </div>
                    </div>

                    {/* Weighed Boxes */}
                    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-2xs">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">WEIGHED BOXES</span>
                      <div className="mt-1 flex items-baseline gap-1">
                        <span className="text-xl font-black text-slate-900">{totalLoadedBoxes} <span className="text-slate-400 font-normal">/ {totalTargetBoxes}</span></span>
                        <span className="text-xs font-semibold text-slate-500">boxes</span>
                      </div>
                      <p className="text-[10px] font-semibold text-emerald-600 mt-0.5">{totalLoadedBoxes} loaded</p>
                    </div>
                  </div>
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm space-y-4">
                    ...
                    <div className="space-y-3 border-b border-slate-100 pb-3">
                      ...
                      <div className="grid grid-cols-3 gap-1 bg-slate-100/80 p-1 rounded-2xl w-full">
                        ...
                        >
                          Pending ({sets.filter(s => !s.loadedWeight || Number(s.loadedWeight) <= 0).length})
                        </button>
                        <button
                        ...
                        <>
                            ...
                            <table className="w-full text-left text-xs">
                              <thead className="bg-slate-50 text-slate-700 font-bold uppercase text-[10px] tracking-wider border-b border-slate-200">
                                <tr>
                                  ...
                                  <th className="px-4 py-3">BOXES IN SET</th>
                                  <th className="px-4 py-3 text-right">EMPTY BOX WT</th>
                                  <th className="px-4 py-3 text-right">LOADED WT</th>
                                  <th className="px-4 py-3 text-right">BIRDS COUNT</th>
                                  <th className="px-4 py-3 text-right">TOTAL NET WT</th>
                                ...
                                {paginatedSets.map((s, sIdx) => {
                                  ...
                                  return (
                                    <tr key={sIdx} className="hover:bg-slate-50/80 transition-colors">
                                      <td className="px-4 py-3 font-bold text-slate-900">Box Set #{s.boxSetNumber || (sIdx + 1)}</td>
                                      <td className="px-4 py-3">
                                        <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-extrabold ${
                                          ...
                                          {isLoaded ? 'Loaded ✓' : 'Pending Load Wt'}
                                        </span>
                                      </td>
                                      <td className="px-4 py-3 font-medium text-slate-700">{boxes} Boxes</td>
                                      <td className="px-4 py-3 text-right text-slate-600">{tare > 0 ? `${tare} kg` : '—'}</td>
                                      <td className="px-4 py-3 text-right font-extrabold text-slate-900">{gross > 0 ? `${gross} kg` : 'Pending'}</td>
                                      <td className="px-4 py-3 text-right font-extrabold text-emerald-700">{birds}</td>
                                      <td className="px-4 py-3 text-right font-extrabold text-emerald-800">{net > 0 ? `${net.toFixed(1)} kg` : '—'}</td>
                                      <td className="px-4 py-3 text-right font-bold text-slate-900">{setAvg > 0 ? `${setAvg} kg` : '—'}</td>
                                    </tr>
                                  );
                                })}
                              </tbody>
                          ...
                          <div className="block md:hidden space-y-3">
                            {paginatedSets.map((s, sIdx) => {
                              ...
                              return (
                                <div key={sIdx} className="rounded-2xl border border-slate-200/90 bg-white p-4 space-y-3 shadow-2xs hover:shadow-xs transition-all">
                                  {/* Set Number & Status Badge Header */}
                                  <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
                                    <div className="flex items-center gap-2 flex-wrap">
                                      <span className="text-sm font-black text-slate-900">
                                        Box Set #{s.boxSetNumber || (sIdx + 1)}
                                      </span>
                                  ...
                                  <div className="grid grid-cols-2 gap-2 text-xs">
                                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-100/90">
                                      ...
                                      <span className="font-extrabold text-slate-900 text-xs block">
                                        {tare > 0 ? `${tare} kg` : '—'} / {isPending ? <em className="text-amber-600 font-normal italic">Pending</em> : `${gross} kg`}
                                      </span>
                                    </div>
                                    ...
                                    <div className={`p-3 rounded-xl border ${isPending ? 'bg-amber-50/80 border-amber-200/60' : 'bg-emerald-50/80 border-emerald-200/60'}`}>
                                      ...
                                      <span className={`font-black block ${isPending ? 'text-amber-900 text-xs' : 'text-emerald-900 text-sm'}`}>
                                        {isPending ? 'Click button below' : `${net.toFixed(1)} kg`}
                                      </span>
                                    </div>
                                  </div>
                                  ...
                                  {isPending ? (
                                    ...
                                    <div className="flex justify-between items-center text-xs pt-2 border-t border-slate-100 text-slate-600 font-semibold">
                                      <span>Birds: <strong className="text-slate-900 font-bold">{birds}</strong></span>
                                      <span>Avg: <strong className="text-slate-900 font-bold">{setAvg > 0 ? `${setAvg} kg/bird` : '—'}</strong></span>
                                    </div>
                                  )}
                                </div>
                              );
    ...
    <div className="space-y-6 pb-12 print:space-y-4 print:pb-0">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div>
            <h2 className="text-lg font-black text-slate-900">All Batches ({batches.length})</h2>
          </div>
            ...
            {[...batches].sort((a, b) => {
              ...
              return (
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] font-semibold text-slate-400">
                          ID: <span className="font-bold text-slate-700">{batchItem.batchNumber || bId}</span>
                        </p>
                      </div>
                    </div>
                  ...
                  {/* Card Meta details */}
                  <div className="grid grid-cols-2 gap-2 text-xs border-y border-slate-100 py-3 my-1 font-medium text-slate-600">
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase block">Farmer</span>
                      <strong className="text-slate-900 font-bold">Ponni Authorized Farmer</strong>
                    </div>
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase block">Placement Date</span>
                      <strong className="text-slate-900 font-bold">{batchItem.startDate || batchItem.placementDate || batchItem.chickPlacementDate || batchItem.arrivalDate || '13/09/2026'}</strong>
                    </div>
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase block">Chick Supplier</span>
                      <strong className="text-slate-800 font-semibold">{batchItem.chickSupplier || 'Ponni Hatcheries'}</strong>
                    </div>
                    <div>
                      <span className="text-[10px] font-bold text-slate-400 uppercase block">Initial Chicks</span>
                      <strong className="text-emerald-800 font-extrabold">{(batchItem.initialChicks || batchItem.initialChickCount || 5000).toLocaleString()}</strong>
                    </div>
                  </div>
        ...
        <>
          ...
          <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs transition-all border-l-4 border-l-emerald-600 print:border-slate-300">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
              <div className="space-y-2">
                ...
                <div>
                  ...
                  <p className="text-xs font-semibold text-slate-500 mt-1 flex items-center gap-1.5">
                    <span>Assigned Farmer:</span>
                    <strong className="text-slate-900 font-extrabold bg-slate-100 px-2 py-0.5 rounded-md">
              ...
              <div className="rounded-xl bg-slate-50/90 border border-slate-200/80 p-4 text-xs space-y-2.5 min-w-[280px]">
                <div className="flex items-center justify-between font-extrabold text-emerald-900 border-b border-slate-200/70 pb-2">
                  <div className="flex items-center gap-2">
                    <div className="rounded-lg bg-emerald-100 p-1 text-emerald-800">
                      <Building2 className="h-4 w-4" />
                    </div>
                    <span>Chick Source & Hatchery</span>
                  </div>
                  <span className="text-[10px] font-bold text-emerald-700 bg-emerald-100/60 px-2 py-0.5 rounded-md">Verified</span>
                </div>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-[11px]">
                  <span className="font-semibold text-slate-500">Hatchery / Source:</span>
                  <span className="font-extrabold text-slate-900 text-right truncate">{b.hatchery || b.supplierName || 'KG Hatcheries'}</span>
                  <span className="font-semibold text-slate-500">Arrival Date:</span>
                  <span className="font-extrabold text-slate-900 text-right">{b.chickArrivalDate || 'N/A'}</span>
                  <span className="font-semibold text-slate-500">Transport Vehicle:</span>
                  <span className="font-extrabold text-slate-900 text-right">{b.vehicleNumber || 'N/A'}</span>
                  <span className="font-semibold text-slate-500">Driver Name:</span>
                  <span className="font-extrabold text-slate-900 text-right">{b.driverName || 'N/A'}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-3 lg:grid-cols-6 print:grid-cols-3">
            {/* Initial Chicks */}
            <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-2xs transition-all hover:border-emerald-300 hover:shadow-xs">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Total Chicks</span>
                <div className="rounded-xl bg-emerald-50 p-2 text-emerald-600 border border-emerald-100">
                  <Bird className="h-4 w-4" />
                </div>
              </div>
              <div className="mt-3">
                <div className="text-2xl font-black text-slate-900 tracking-tight">{historyData.initialChickCount.toLocaleString()}</div>
            ...
            {(() => {
              ...
              return (
                <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-2xs transition-all hover:border-blue-300 hover:shadow-xs">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Final Birds</span>
                    <div className="rounded-xl bg-blue-50 p-2 text-blue-600 border border-blue-100">
                      <ShieldCheck className="h-4 w-4" />
                    </div>
                  </div>
                  <div className="mt-3">
                    <div className="text-2xl font-black text-slate-900 tracking-tight">{finalBirdsCount.toLocaleString()}</div>
                    <div className="mt-1 flex items-center gap-1 text-[10px] font-extrabold text-blue-700">
                      <span className="rounded bg-blue-50 px-1.5 py-0.5 border border-blue-100">
                        {(100 - parseFloat(historyData.mortalityPercentage)).toFixed(1)}% Live Rate
                      </span>
                    </div>
                  </div>
                </div>
            ...
            {/* Total Mortality */}
            <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-2xs transition-all hover:border-rose-300 hover:shadow-xs">
              <div className="flex items-center justify-between">
              ...
              <div className="mt-3">
                <div className="text-2xl font-black text-rose-700 tracking-tight">{historyData.totalMortality.toLocaleString()}</div>
                <div className="mt-1 flex items-center gap-1 text-[10px] font-extrabold text-rose-600">
                  <span className="rounded bg-rose-50 px-1.5 py-0.5 border border-rose-100">
                    {historyData.mortalityPercentage}% Mortality Rate
                  </span>
                </div>
              </div>
            </div>
            ...
            {/* Feed Consumed */}
            <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-2xs transition-all hover:border-amber-300 hover:shadow-xs">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Feed Consumed</span>
              ...
              <div className="mt-3">
                <div className="text-2xl font-black text-slate-900 tracking-tight">
                  {historyData.totalFeedBags} <span className="text-xs font-bold text-slate-500">bags</span>
                </div>
                <div className="mt-1 text-[10px] font-semibold text-amber-700">
                  {historyData.totalFeedConsumedKg.toLocaleString()} kg total feed
                </div>
              </div>
            </div>

            {/* Final Dispatched Weight */}
            <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-2xs transition-all hover:border-purple-300 hover:shadow-xs">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Final Weight</span>
              ...
              <div className="mt-3">
                ...
                <div className="mt-1 text-[10px] font-semibold text-purple-700">
                  {historyData.totalDispatchedBirds.toLocaleString()} birds dispatched
                </div>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-2xs transition-all hover:border-emerald-300 hover:shadow-xs">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Avg Bird Wt</span>
              ...
              <div className="mt-3">
                ...
                <div className="mt-1 text-[10px] font-extrabold text-emerald-700">
                  {historyData.avgBirdWeight > 0 ? `${(historyData.avgBirdWeight * 1000).toFixed(0)} g / bird` : 'No weight data'}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="p-2 bg-slate-100/70 border-b border-slate-200/80 print:hidden">
              <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none whitespace-nowrap">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-extrabold transition-all cursor-pointer shrink-0 whitespace-nowrap ${
                    activeTab === 'traders'
                      ? 'bg-emerald-700 text-white shadow-sm shadow-emerald-700/30'
                ...
                >
                  ...
                  <span className="hidden sm:inline">Total Birds & Weight Per Shop / Trader</span>
                  <span className="sm:hidden">Shops / Traders</span>
                ...
                >
                  ...
                  <span className="hidden sm:inline">Feed Stock Loaded History</span>
                ...
                >
                  ...
                  <span className="hidden sm:inline">Medicine & Vaccination Log</span>
                ...
                >
                  ...
                  <span className="hidden sm:inline">Daily Growth & Mortality Log</span>
                    ...
                    <div className={activeTab === 'traders' ? 'p-4 sm:p-6 space-y-4' : 'hidden print:block p-4 sm:p-6 space-y-4'}>
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-4">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-sm sm:text-base font-bold text-slate-900">Per Shop / Trader Dispatch History</h3>
                        </div>
                        <span className="text-[11px] sm:text-xs font-semibold text-slate-500">
                          Click any trader to view detailed box set records
                        </span>
                      </div>

                      {historyData.traderBreakdown.length === 0 ? (
                        <div className="rounded-xl border border-dashed border-slate-200 p-8 text-center text-xs text-slate-400">
                          No dispatches recorded for this batch yet.
                        </div>
                      ) : (
                        <>
                          {/* DESKTOP VIEW TABLE */}
                            ...
                            <table className="w-full text-left text-xs">
                              <thead className="bg-slate-50 text-slate-700 font-bold uppercase text-[10px] tracking-wider border-b border-slate-200">
                                <tr>
                                  ...
                                  <th className="px-4 py-3 text-right">Dispatches</th>
                                  <th className="px-4 py-3 text-right">Total Birds Given</th>
                                  <th className="px-4 py-3 text-right">Total Weight (Kg)</th>
                                  <th className="px-4 py-3 text-right">Avg Weight / Bird</th>
                                  <th className="px-4 py-3 text-right">Total Crates</th>
                              ...
                              <tbody className="divide-y divide-slate-200 bg-white">
                                {paginatedTraders.map((t, idx) => (
                                  <tr
                                    ...
                                    title="Click to view complete box & set breakdown"
                                  >
                                    <td className="px-4 py-3.5 font-bold text-slate-900 flex items-center gap-2">
                                      <div className="h-7 w-7 rounded-lg bg-emerald-100 text-emerald-800 flex items-center justify-center font-black text-xs group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                                        {t.traderName.charAt(0).toUpperCase()}
                                      </div>
                                      <span className="group-hover:text-emerald-800 font-black">{t.traderName}</span>
                                    </td>
                                    <td className="px-4 py-3.5 text-right font-medium text-slate-600">{t.dispatchCount}</td>
                                    <td className="px-4 py-3.5 text-right font-extrabold text-slate-900">{t.totalBirds.toLocaleString()} birds</td>
                                    <td className="px-4 py-3.5 text-right font-black text-emerald-800">{t.totalWeight.toLocaleString()} kg</td>
                                    <td className="px-4 py-3.5 text-right font-semibold text-purple-700">{t.avgWeight} kg</td>
                                    <td className="px-4 py-3.5 text-right font-medium text-slate-600">{t.totalCrates}</td>
                                    <td className="px-4 py-3.5 text-center print:hidden">
                                      <div className="flex items-center justify-center gap-2">
                                        <button
                                          ...
                                          className="inline-flex items-center gap-1 rounded-lg bg-emerald-100 hover:bg-emerald-600 hover:text-white px-2.5 py-1 text-[11px] font-extrabold text-emerald-900 transition-colors shadow-2xs cursor-pointer"
                                        ...
                                        <span className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                                          <span>View Sets</span>
                              ...
                              <tfoot className="bg-slate-100 font-extrabold text-slate-900 border-t-2 border-slate-300">
                                <tr>
                                  <td className="px-4 py-3">Total / Grand Summary</td>
                                  <td className="px-4 py-3 text-right">{historyData.dispatches.length}</td>
                                  <td className="px-4 py-3 text-right text-emerald-900">{historyData.totalDispatchedBirds.toLocaleString()} birds</td>
                                  <td className="px-4 py-3 text-right text-emerald-900">{historyData.totalDispatchedWeight.toLocaleString()} kg</td>
                                  ...
                                  <td className="px-4 py-3 text-right">{historyData.totalCratesCount}</td>
                            ...
                            {paginatedTraders.map((t, idx) => (
                              ...
                              >
                                ...
                                <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-2.5">
                                  <span className="rounded-md bg-emerald-50 border border-emerald-100 px-2 py-0.5 text-[10px] font-extrabold text-emerald-800 whitespace-nowrap shrink-0">
                                    {t.dispatchCount} Dispatch{t.dispatchCount > 1 ? 'es' : ''} • {t.totalCrates} boxes
                                  </span>
                                  ...
                                  <div className="flex items-center gap-1.5 shrink-0 whitespace-nowrap">
                                    <button
                                      ...
                                      className="inline-flex items-center gap-1 rounded-lg bg-emerald-100 hover:bg-emerald-200 px-2.5 py-1 text-[11px] font-extrabold text-emerald-900 active:scale-95 transition-all cursor-pointer whitespace-nowrap shrink-0"
                                    >
                                      <FileText className="h-3.5 w-3.5 shrink-0" />
                                      <span className="whitespace-nowrap">Invoice</span>
                                    </button>
                                    <span className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700 whitespace-nowrap shrink-0">
                                      <span className="whitespace-nowrap">View Sets</span>
                                ...
                                <div className="flex items-center gap-2.5 py-0.5">
                                  ...
                                  <div className="min-w-0 flex-1">
                                    <h4 className="font-extrabold text-slate-900 text-sm leading-snug break-words" title={t.traderName}>
                                      {t.traderName}
                                    </h4>
                                  </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                  <div className="rounded-xl bg-slate-50 p-2.5 border border-slate-100">
                                    <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400 block">TOTAL BIRDS GIVEN</span>
                                    <span className="font-extrabold text-slate-900 text-xs mt-0.5 block">{t.totalBirds.toLocaleString()} birds</span>
                                  </div>

                                  <div className="rounded-xl bg-emerald-50/80 p-2.5 border border-emerald-100">
                                    <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-700 block">TOTAL WEIGHT</span>
                                    <span className="font-black text-emerald-800 text-xs mt-0.5 block">{t.totalWeight.toLocaleString()} kg</span>
                                  </div>
                                </div>

                                <div className="flex items-center justify-between pt-1 border-t border-slate-100 text-xs font-semibold text-slate-600">
                                  <span>Avg Wt: <strong className="text-purple-700 font-bold">{t.avgWeight} kg/bird</strong></span>
                                  <span>Crates: <strong className="text-slate-900 font-bold">{t.totalCrates} boxes</strong></span>
                                </div>
                              </div>
                            ))}
                          </div>
                    ...
                    <div className="p-4 sm:p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-sm sm:text-base font-bold text-slate-900">Feed Stock Loaded & Arrival History</h3>
                        </div>
                      </div>

                      {historyData.feedArrivals.length === 0 ? (
                        <div className="rounded-xl border border-dashed border-slate-200 p-8 text-center text-xs text-slate-400">
                          No feed stock loaded or arrival entries logged for this batch.
                        </div>
                      ) : (
                        <>
                          {/* DESKTOP VIEW TABLE */}
                            ...
                            <table className="w-full text-left text-xs">
                              ...
                              <tbody className="divide-y divide-slate-200 bg-white">
                                {paginatedFeed.map((f, idx) => {
                                  ...
                                  const feedTypeStr = f.feedType || 'Feed';
                                  ...
                                  return (
                                    <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                                      <td className="px-4 py-3 font-semibold text-slate-900">{dateStr}</td>
                                      <td className="px-4 py-3 font-extrabold text-emerald-900">{feedTypeStr}</td>
                                      <td className="px-4 py-3">
                                        <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-[10px] font-extrabold ${
                                          isReturn ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
                                        }`}>
                                          {isReturn ? 'Return (-)' : 'Received (+)'}
                                        </span>
                                      </td>
                                      <td className="px-4 py-3 text-right font-extrabold text-slate-900">{bags} bags</td>
                                      <td className="px-4 py-3 text-right font-medium text-slate-600">{extraKg > 0 ? `${extraKg} kg` : '—'}</td>
                                      <td className="px-4 py-3 text-right font-black text-emerald-800">{totalKg.toLocaleString()} kg</td>
                                      <td className="px-4 py-3 font-medium text-slate-700">{vehicleStr}</td>
                                      <td className="px-4 py-3 text-slate-500 italic">{notesStr}</td>
                                    </tr>
                                  );
                                })}
                              </tbody>
                                ...
                                <div key={idx} className="rounded-2xl border border-slate-200 bg-white p-4 space-y-2.5 shadow-2xs">
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="font-extrabold text-xs text-slate-900">{dateStr}</span>
                                    </div>
                                  ...
                                  <div className="rounded-xl bg-slate-50 p-3 border border-slate-100 space-y-1.5">
                                    <div className="flex items-center justify-between">
                                      ...
                                      <span className="text-xs font-black text-slate-900">{bags} bags ({totalKg} kg)</span>
                                    </div>
                                    {extraKg > 0 && (
                                      <div className="text-[11px] font-semibold text-slate-500">Includes {extraKg} kg loose feed</div>
                                    )}
                                  </div>

                                  <div className="flex flex-wrap items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-100">
                                    <span>Vehicle/Driver: <strong className="text-slate-800 font-bold">{vehicleStr}</strong></span>
                    ...
                    <div className="p-4 sm:p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-sm sm:text-base font-bold text-slate-900">Medicine & Vaccine Log History</h3>
                        </div>
                      </div>

                      {historyData.medicineRecords.length === 0 ? (
                        <div className="rounded-xl border border-dashed border-slate-200 p-8 text-center text-xs text-slate-400">
                          No medicine or vaccination entries logged for this batch.
                        </div>
                      ) : (
                        <>
                          {/* DESKTOP VIEW TABLE */}
                          <div className="hidden md:block overflow-x-auto rounded-xl border border-slate-200">
                            <table className="w-full text-left text-xs">
                              <thead className="bg-slate-50 text-slate-700 font-bold uppercase text-[10px] tracking-wider border-b border-slate-200">
                                <tr>
                                  <th className="px-4 py-3">Date</th>
                                  <th className="px-4 py-3">Bird Age</th>
                                  <th className="px-4 py-3">Medicine / Vaccine Name</th>
                                  <th className="px-4 py-3">Dosage / Quantity</th>
                              ...
                              <tbody className="divide-y divide-slate-200 bg-white">
                                {paginatedMedicines.map((m, idx) => {
                                  ...
                                  if (!medNameStr || medNameStr === '—') {
                                    medNameStr = m.medicineName || m.vaccineName || m.name || m.reason || 'Medication Log';
                                  }
                                  ...
                                  return (
                                    <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                                      <td className="px-4 py-3 font-semibold text-slate-900">{dateStr}</td>
                                      <td className="px-4 py-3 font-medium text-slate-600">{birdAgeStr}</td>
                                      <td className="px-4 py-3 font-extrabold text-emerald-900">{medNameStr}</td>
                                      <td className="px-4 py-3 font-semibold text-slate-800">{dosageStr}</td>
                                      <td className="px-4 py-3 font-medium text-slate-600">{adminStr}</td>
                                      <td className="px-4 py-3 text-slate-500 italic">{notesStr}</td>
                                    </tr>
                                  );
                                })}
                              ...
                              return (
                                <div key={idx} className="rounded-2xl border border-slate-200 bg-white p-4 space-y-2.5 shadow-2xs">
                                  ...
                                  <div className="rounded-xl bg-slate-50 p-3 border border-slate-100 space-y-1">
                                    <div className="text-xs font-black text-emerald-900">{medNameStr}</div>
                                    <div className="text-xs font-semibold text-slate-700">Dosage: <span className="text-slate-900 font-extrabold">{dosageStr}</span></div>
                                  </div>

                                  <div className="flex flex-wrap items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-100">
                                    <span>Admin By: <strong className="text-slate-800 font-bold">{adminStr}</strong></span>
                                    {notesStr !== '—' && <span className="italic text-slate-400">{notesStr}</span>}
                                  </div>
                                </div>
                              );
                    ...
                    <div className="p-4 sm:p-6 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <h3 className="text-sm sm:text-base font-bold text-slate-900">Daily Farm Log History</h3>
                        </div>
                      </div>

                      {historyData.dailyRecords.length === 0 ? (
                        <div className="rounded-xl border border-dashed border-slate-200 p-8 text-center text-xs text-slate-400">
                          No daily records logged for this batch yet.
                        </div>
                      ) : (
                        <>
                          {/* DESKTOP VIEW TABLE */}
                          <div className="hidden md:block overflow-x-auto rounded-xl border border-slate-200">
                            <table className="w-full text-left text-xs">
                              <thead className="bg-slate-50 text-slate-700 font-bold uppercase text-[10px] tracking-wider border-b border-slate-200">
                                <tr>
                                  <th className="px-4 py-3">Date</th>
                                  <th className="px-4 py-3 text-right">Mortality Count</th>
                                  <th className="px-4 py-3">Feed Type</th>
                                  <th className="px-4 py-3 text-right">Feed Consumed (Kg)</th>
                                  <th className="px-4 py-3 text-right">Average Wt (Grams)</th>
                                  <th className="px-4 py-3 text-right">Live Birds Left</th>
                              ...
                              <tbody className="divide-y divide-slate-200 bg-white">
                                {paginatedDaily.map((r, idx) => (
                                  <tr
                                    ...
                                    title="Click to view full record breakdown"
                                  >
                                    <td className="px-4 py-3 font-bold text-slate-900">{r.recordDate}</td>
                                    <td className="px-4 py-3 text-right font-extrabold text-rose-600">{r.mortalityCount || 0}</td>
                                    <td className="px-4 py-3 font-semibold text-slate-700">{r.feedType || 'Pre-Starter'}</td>
                                    <td className="px-4 py-3 text-right font-black text-amber-700">{r.feedConsumption || 0} kg</td>
                                    <td className="px-4 py-3 text-right font-bold text-purple-700">{r.averageWeight || 0} g</td>
                                    <td className="px-4 py-3 text-right font-extrabold text-slate-900">{r.remainingChickCount ?? '—'}</td>
                                    <td className="px-4 py-3 text-center">
                                      <span className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700 hover:bg-emerald-100 transition-colors">
                                ...
                                >
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <span className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-700">
                                      Live: {typeof chickCount === 'number' ? chickCount.toLocaleString() : chickCount}
                                    </span>
                                  </div>
      ...
      <Modal
        ...
        title={`Daily Farm Record (${viewingDailyRecord?.recordDate})`}
      >
        {viewingDailyRecord && (() => {
          let viewingAgeDay = 1;
          ...
          return (
            <div className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="rounded-xl bg-rose-50/80 p-3 border border-rose-100/80 text-center">
                  <span className="text-[10px] font-extrabold uppercase tracking-wider text-rose-500 block mb-0.5">Mortality</span>
                  <span className="text-base sm:text-lg font-black text-rose-700 block">{viewingDailyRecord.mortalityCount || 0} birds</span>
                  <span className="text-[10px] font-medium text-rose-600 block mt-0.5">Live: {viewingDailyRecord.remainingChickCount ?? '—'}</span>
                </div>

                <div className="rounded-xl bg-amber-50/80 p-3 border border-amber-100/80 text-center">
                  <span className="text-[10px] font-extrabold uppercase tracking-wider text-amber-600 block mb-0.5">Feed Consumed</span>
                  <span className="text-base sm:text-lg font-black text-amber-800 block">{feedKg} kg</span>
                  <span className="text-[10px] font-medium text-amber-700 block mt-0.5">({feedBags} Bags)</span>
                </div>

                <div className="rounded-xl bg-purple-50/80 p-3 border border-purple-100/80 text-center">
                  <span className="text-[10px] font-extrabold uppercase tracking-wider text-purple-600 block mb-0.5">Avg Weight</span>
                  <span className="text-base sm:text-lg font-black text-purple-800 block">{viewingDailyRecord.averageWeight || 0} g</span>
                  <span className="text-[10px] font-medium text-purple-700 block mt-0.5">({weightDiff > 0 ? '+' : ''}{weightDiff}g vs target)</span>
                </div>
              </div>
